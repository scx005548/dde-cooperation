...
void sha256Count(Sha256State *state, const void *data) {
    ...
    size_t i = 0;
    for (; i < 16; i++) {
        DATA[i] = readAsBigEndian32(((uint8_t *)data) +
                                    i * 4); // changeEndian32(((uint32_t*)data)[i]);
    }

    for (; i < 64; i++) {
        uint32_t S0 = rotateRight32(DATA[i - 15], 7) ^ rotateRight32(DATA[i - 15], 18) ^
                      (DATA[i - 15] >> 3);
        uint32_t S1 = rotateRight32(DATA[i - 2], 17) ^ rotateRight32(DATA[i - 2], 19) ^
                      (DATA[i - 2] >> 10);
