...
#define SERVER_DEBUGGER_PORT "5005"
    ...
    args
        << QString("lock_video_orientation=%1").arg(QString::number(m_params.lockVideoOrientation));
    ...
    // adb -s P7C0218510000537 shell CLASSPATH=/data/local/tmp/scrcpy-server app_process /
    // com.genymobile.scrcpy.Server 0 8000000 false mark: crop input format: "width:height:x:y" or
    // "" for no crop, for example: "100:200:0:0"
            ...
            case SSS_ENABLE_TUNNEL_REVERSE:
                if (qsc::AdbProcess::AER_SUCCESS_EXEC == processResult) {
                    ...
                    if (!m_serverSocket.listen(QHostAddress::LocalHost, m_params.localPort)) {
                        qCritical() << QString("Could not listen on port %1")
                                           .arg(m_params.localPort)
                                           .toStdString()
                                           .c_str();
                        m_serverStartStep = SSS_NULL;
                        disableTunnelReverse();
