...
void InputConvertNormal::mouseEvent(const QMouseEvent *from,
    ...
    controlMsg->setInjectTouchMsgData(static_cast<quint64>(POINTER_ID_MOUSE),
                                      action,
...
void InputConvertNormal::wheelEvent(const QWheelEvent *from,
                                    const QSize &frameSize,
                                    const QSize &showSize) {
    ...
    // delta
    qint32 hScroll = from->angleDelta().x() == 0
                         ? 0
                         : from->angleDelta().x() / abs(from->angleDelta().x()) * 2;
    qint32 vScroll = from->angleDelta().y() == 0
                         ? 0
                         : from->angleDelta().y() / abs(from->angleDelta().y()) * 2;
...
void InputConvertNormal::keyEvent(const QKeyEvent *from,
                                  const QSize &frameSize,
                                  const QSize &showSize) {
    Q_UNUSED(frameSize)
    Q_UNUSED(showSize)
    ...
    controlMsg->setInjectKeycodeMsgData(action,
                                        keyCode,
                                        m_repeat,
