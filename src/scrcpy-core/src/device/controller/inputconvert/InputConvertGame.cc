...
InputConvertGame::InputConvertGame(Controller *controller)
    ...
    connect(m_ctrlSteerWheel.delayData.timer,
            &QTimer::timeout,
            this,
            &InputConvertGame::onSteerWheelTimer);
}
...
void InputConvertGame::processSteerWheel(const KeyMap::KeyMapNode &node, const QKeyEvent *from) {
    ...
    if (pressedNum == 0) {
        ...
        sendTouchUpEvent(getTouchID(m_ctrlSteerWheel.touchKey),
    ...
    if (pressedNum == 1 && flag) {
        ...
        getDelayQueue(m_ctrlSteerWheel.delayData.currentPos,
                      node.data.steerWheel.centerPos + offset,
                      0.01f,
                      0.002f,
                      2,
...
bool InputConvertGame::processMouseMove(const QMouseEvent *from) {
    ...
    if (!m_ctrlMouseMove.lastPos.isNull() && m_processMouseMove) {
        ...
        m_ctrlMouseMove.lastConverPos.setX(m_ctrlMouseMove.lastConverPos.x() +
                                           distance.x() / m_showSize.width());
        m_ctrlMouseMove.lastConverPos.setY(m_ctrlMouseMove.lastConverPos.y() +
...
void InputConvertGame::mouseMoveStartTouch(const QMouseEvent *from) {
    ...
    if (!m_ctrlMouseMove.touching) {
        QPointF mouseMoveStartPos = m_ctrlMouseMove.smallEyes
